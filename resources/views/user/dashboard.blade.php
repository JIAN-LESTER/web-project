...
<div class="col-sm-6 col-xl-3" style="--i: 3">
  <div class="card border-danger rounded-3 mb-2 stat-card">
    <div class="card-body d-flex align-items-center p-2">
      ...
      <div>
        <h5 class="mb-0 text-danger fw-bold">{{ $unAnsweredMessages }}</h5>
        <small class="text-muted">Unanswered Messages</small>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="card shadow rounded-3 mb-1 logs-section" style="margin-top:15px;">
    <div class="card-header d-flex justify-content-between align-items-center">
      <div class="d-flex align-items-center">
        <h5 class="mb-0 fw-bold">Recent Inquiry Logs</h5>
      </div>
    </div>
    ...
    {{ $log->message?->content ?? '[No message content]' }}
</div>
          </div>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  items.forEach(item => {
    item.addEventListener('click', function() {
      ...
      window.location.href = "{{ route('user.dashboard') }}?filter=" + value;
    });
  });
